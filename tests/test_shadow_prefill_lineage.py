...
"""Characterization tests for shadow-prefill *lineage* handling.
...
from omlx.shadow_prefill import ShadowJob
...
def _target_for(prompt_len: int) -> int:
    """What `note_shadow_candidate` will set `target_tokens` to.
...
def _lineage_spies(scheduler):
    ...
    `extend` and `_shadow_drop_job` keep doing exactly what they did; the
    wrappers only tally. `omlx.scheduler.ShadowJob` is rebound rather than
    mocked out, so every job under test is a real `ShadowJob`.
    """
    ...
    real_extend = ShadowJob.extend
    real_drop = scheduler._shadow_drop_job
    ...
    def counting_ctor(*args, **kwargs):
        ...
        return ShadowJob(*args, **kwargs)

    with patch.object(ShadowJob, "extend", counting_extend), patch.object(
        scheduler, "_shadow_drop_job", counting_drop
    ), patch("omlx.scheduler.ShadowJob", counting_ctor):
        yield counts


def _publish(scheduler, job, boundary, state_tokens, block_ids, readback=None):
    """Run `_shadow_publish` with everything below the decision stubbed out.
    ...
    with patch.object(
        ...
        scheduler._shadow_publish(job, boundary, state)
    return worker
...
class TestB2RapidTurnsCoalesce:
    ...
    def test_four_growing_turns_are_one_job_created_once_and_extended_three_times(self):
        ...
        with _lineage_spies(scheduler) as counts:
            for turn, prompt_len in enumerate(self.PROMPTS):
                ...
                identities.append(id(scheduler._shadow_job))
    ...
    def test_an_append_inside_the_same_block_neither_replaces_nor_extends(self):
        ...
        `note_shadow_candidate` returns first — so `extend_refusals` stays 0
...
class TestB3SharedPrefixFork:
    ...
    def test_the_drop_that_replaces_a_job_is_the_replaced_reason(self):
        ...
        with patch.object(
            scheduler, "_shadow_drop_job", wraps=scheduler._shadow_drop_job
        ) as drop:
            scheduler.note_shadow_candidate(
                _sparse_request_on(branch_b, rid="b1", scheduler=scheduler)
            )
    ...
    def test_a_longer_fork_is_refused_on_content_rather_than_on_length(self):
        ...
        assert scheduler._shadow_job is not job_a
        assert scheduler._shadow_job.tokens[: len(common)] == common
        assert set(scheduler._shadow_job.tokens).isdisjoint(branch_a[len(common):])
    ...
    def test_a_replaced_job_keeps_what_it_published(self):
        ...
        release.assert_called_once_with("shadow:a1")
        drop_snapshots.assert_called_once_with("shadow:a1")
    ...
    def test_two_branches_are_stored_under_different_token_keys(self):
        ...
        `_shadow_publish` keys the store by `list(job.tokens[:boundary])`, so
        ...
        assert worker_a.call_args.args[0] == "shadow:a1"
        assert worker_b.call_args.args[0] == "shadow:b1"
...
class TestB4CompactionDivergence:
    ...
    def test_nothing_is_published_for_the_old_lineage_across_the_divergence(self):
        scheduler = _make_scheduler()
        old = self._started_job(scheduler)
        with patch.object(scheduler, "_shadow_publish") as publish:
            scheduler.note_shadow_candidate(
                _sparse_request_on(self._compacted(), rid="r2", scheduler=scheduler)
            )
    ...
    def test_an_extension_mid_chunk_publishes_the_old_boundary_of_the_same_lineage(self):
        """The branch `_shadow_step_inner` takes when the target moved under it.
        ...
        state = MagicMock(
            ...
            cache=[MagicMock()],
            shadow_target_tokens=old_target,
        )
        ...
        ), patch.object(scheduler, "_shadow_publish") as publish:
    ...
    def test_a_replacement_mid_chunk_publishes_nothing(self):
        ...
        `_shadow_step_inner` reads `self._shadow_job` once, at the top. A
        ...
        Nothing is published only because `_shadow_drop_job` sets `cancelled`
        ...
        state = MagicMock(
            ...
            tokens_processed=2 * BLOCK,
            cache=[MagicMock()],
            shadow_target_tokens=old.target_tokens,
        )
    ...
    def test_a_swap_between_steps_is_seen_because_the_step_re_reads_the_job(self):
        ...
        `job = self._shadow_job` at the top of `_shadow_step_inner` is read
        ...
        new_state = MagicMock(
            ...
            cache=[MagicMock()],
            shadow_target_tokens=new.target_tokens,
        )
        begun_for = []
        stepped = []
